...
for domain in domains:
    for metric in metrics:
        ...
        data_dict = {}
        metric_path = f'{domain}/sys/sys_{metric}.tsv'

        if os.path.exists(metric_path):
            continue

        if metric in ['BLEURT-20','COMET-MQM_2021','COMET-QE-MQM_2021']:

            for file_name in os.listdir(f'{domain}/{metric}'):
                if file_name.endswith('.tsv'):
                    df = pd.read_csv(f'{domain}/{metric}/{file_name}', sep='\t') 
            ...
            metric_data.to_csv(f'{domain}/sys/sys_{metric}.tsv', sep='\t', index=False)

        if metric in ['sacre_BLEU','sacre_TER','sacre_CHRF2']:
            for file_name in os.listdir(f'Data/{domain}'):
                if file_name.endswith('.tsv'):

                    df = pd.read_csv(f'{domain}/{file_name}', sep='\t') 
                    segment_scores = list(df[metric])
                    system_score = sum(segment_scores) / len(segment_scores)
                    data_dict[file_name[:-4]] = system_score
