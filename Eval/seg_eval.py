...
def compute_seg_correlation(domain, metric, human_judgments):
    """
    ...
    :param sting human_judgments: human judgment type ('mqm', 'raw', 'z') 
    :return: None
    """
        ...
        for file in os.listdir(path):
            if file.endswith('.tsv'):
                ...
                if metric in ['BLEURT-20', 'COMET-MQM_2021']:
                    ...
                    if human_judgments == 'mqm':
                        if domain == 'newstest2021':
                            ref_A = list(file_df[f'{metric}_ref_A'])
                            ref_B = list(file_df[f'{metric}_ref_B'])
                            human_ratings_df = pd.read_csv(f'human_judgments_seg/all_news_seg_{human_judgments}_scores.tsv', sep='\t', on_bad_lines='skip', keep_default_na=False)
